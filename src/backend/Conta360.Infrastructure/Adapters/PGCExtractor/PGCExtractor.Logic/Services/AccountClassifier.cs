...
namespace Conta360.Infrastructure.Adapters.PGCExtractor.PGCExtractor.Logic.Services
{
    ...
    {
        public void ClassifyAccounts(List<AccountNode> accounts)
        {
            foreach (var acc in accounts)
            {
                if (acc.Code.StartsWith("1") || acc.Code.StartsWith("2"))
                    ...
                    acc.Category = "Existencias";
                else if (acc.Code.StartsWith("4") || acc.Code.StartsWith("5"))
                    acc.Category = "Pasivo";
                else if (acc.Code.StartsWith("6"))
                    acc.Category = "Gasto";
                else if (acc.Code.StartsWith("7"))
                    acc.Category = "Ingreso";
                else if (acc.Code.StartsWith("8"))
                    acc.Category = "Gasto Patrimonio";
                else if (acc.Code.StartsWith("9"))
                    acc.Category = "Ingreso Patrimonio";
            }
        }
    }
}

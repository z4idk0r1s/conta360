/*
        ...
        private static SancionDto MapSancionApiModelToDto(SancionApiModel apiModel) => new() { Id = apiModel.Id, Nombre = apiModel.Nombre, Motivo = apiModel.Motivo, Sancion = apiModel.Sancion, Estado = apiModel.Estado };
        private static SancionDetalleDto MapSancionDetalleApiModelToDto(SancionDetalleApiModel apiModel) => new() { Id = apiModel.Id, Nombre = apiModel.Nombre, Motivo = apiModel.Motivo, Sancion = apiModel.Sancion, Estado = apiModel.Estado, Detalles = apiModel.Detalles, FechaResolucion = apiModel.FechaResolucion, OrganismoId = apiModel.OrganismoId };
...
        public async Task<List<OrganoDto>> GetOrganosAsync() => _mapper.Map<List<OrganoDto>>(await GetFromApiAsync<OrganoApiModel>("organos"));
